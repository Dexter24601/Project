...
from RecognitionSystem.utils.general import (check_img_size, cv2, increment_path, non_max_suppression, scale_coords, Profile)
...
def resource_path(relative_path):
    ...
    if not os.path.exists(Parent):
        os.makedirs(os.path.dirname(os.path.join(base_path, relative_path).replace('\\', '/')))
        ...
        for i, det in enumerate(pred):  # Every detection on every image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    CroppedImg = save_one_box(xyxy,DetectedImage,pad=padx, BGR=True, save=False)
                    ...
                    cv2.imwrite(f'{DetectedPath}/{len(os.listdir(DetectedPath))}.jpg', T_Cropped)
                    ...
                    cv2.imwrite(f'{NoBGPath}/{len(os.listdir(DetectedPath))}.jpg', T_NoBG)
